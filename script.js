...
function draw(){
  ...
  ctx.fillStyle=isDark?'#0d0d12':'#fdf8f0'; ctx.fillRect(0,0,W,H);
  orbs.forEach((od,i)=>{
...
/* ── Toggles: handled by window.toggleSwitch defined at bottom ── */
function _toggleSwitchImpl(key){const el=document.getElementById('toggle-'+key);if(el)el.classList.toggle('on');}
...
const GESTURES = [
  // Mouse Gestures
  {group:'Mouse Gestures', name:'Move',         desc:'Move your open hand in any direction to control the cursor position on screen.',        frames:['frames/open-palm.png']},
  {group:'Mouse Gestures', name:'Right Click',  desc:'Touch your thumb and middle finger together to activate the right-click command.',      frames:['frames/open-palm.png','frames/right-click.png']},
  {group:'Mouse Gestures', name:'Left Click',   desc:'Touch your index finger tip to your thumb to perform a left click.',                    frames:['frames/open-palm.png','frames/left-click.png']},
  {group:'Mouse Gestures', name:'Double Click', desc:'Quickly tap your index finger to your thumb twice in rapid succession.',                frames:['frames/open-palm.png','frames/open-palm.png','frames/left-click.png']},
  {group:'Mouse Gestures', name:'Drag',         desc:'Pinch your thumb and index finger together and move your hand to drag an element.',     frames:['frames/open-palm.png','frames/left-click.png','frames/left-click.png','frames/open-palm.png']},
  // Keyboard Gestures
  {group:'Keyboard Gestures', name:'Mode Switch', desc:'Form a fist and hold briefly to toggle between mouse mode and keyboard mode.',        frames:['frames/open-palm.png', 'frames/keyboard.png']},
  {group:'Keyboard Gestures', name:'Press Key',   desc:'Extend a single finger and move it toward a virtual key to press it.',               frames:['frames/open-palm.png','frames/left-click.png','frames/open-palm.png','frames/left-click.png']},
  {group:'Keyboard Gestures', name:'Backspace',   desc:'Swipe your open hand quickly to the left to trigger the backspace key.',             frames:['frames/open-palm.png','frames/right-click.png']},
];
...
/* ── Camera widget close ── */
function closeCamWidget(){
  document.getElementById('cam-widget').classList.remove('open');
  setIslandActive('island-camera-btn', false);
  setMiniCamActive(false);
}

/* ── Camera feed hide/show ── */
let camFeedHidden = false;
function toggleCamFeed(){
  camFeedHidden = !camFeedHidden;
  const feed = document.getElementById('cam-feed-area');
  const btn  = document.getElementById('cam-hide-btn');
  if(feed) feed.style.display = camFeedHidden ? 'none' : '';
  if(btn)  btn.style.color = camFeedHidden ? 'rgba(98,114,255,0.8)' : '';
  btn.title = camFeedHidden ? 'Show camera feed' : 'Hide camera feed';
}

/* ── Island cam center feed toggle ── */
let iccFeedHidden = false;
function toggleIslandCamFeed(){
  iccFeedHidden = !iccFeedHidden;
  const feed = document.getElementById('icc-feed');
  const btn  = document.getElementById('icc-eye-btn');
  if(feed) feed.style.display = iccFeedHidden ? 'none' : '';
  btn.title = iccFeedHidden ? 'Show feed' : 'Hide feed';
  btn.style.color = iccFeedHidden ? 'rgba(0,255,220,0.7)' : '';
}
function closeIslandCam(){
  document.getElementById('island-cam-center').classList.remove('open');
  miniCamOn = false;
  setIslandActive('island-camera-btn', false);
}

/* ── Mini island ── */
let miniIslandOpen = false;
function toggleMiniIsland(){
  miniIslandOpen = !miniIslandOpen;
  document.getElementById('mini-island').classList.toggle('open', miniIslandOpen);
  setIslandActive('island-mini-btn', miniIslandOpen);
  const app = document.getElementById('app');
  if(miniIslandOpen){
    app.classList.add('launcher-hidden');
    // show skeleton if toggled on
    if(!miniSkelOn) document.getElementById('skeleton-overlay').classList.add('visible');
  } else {
    app.classList.remove('launcher-hidden');
...
let miniCamOn = false;
function toggleMiniCamera(){
  miniCamOn = !miniCamOn;
  const btn = document.getElementById('mini-cam-btn');
  // in island mode: use centered cam panel
  const icc = document.getElementById('island-cam-center');
  if(miniCamOn){
    icc.classList.add('open');
    btn.classList.remove('active-cam');
    btn.title = 'Hide Camera';
    setIslandActive('island-camera-btn', true);
  } else {
    icc.classList.remove('open');
    btn.classList.add('active-cam');
    btn.title = 'Show Camera';
    setIslandActive('island-camera-btn', false);
  }
  setMiniCamActive(!miniCamOn);
}

function setMiniCamActive(slashed){
  const slash = document.getElementById('mini-cam-slash');
  if(slash) slash.style.display = slashed ? '' : 'none';
}

let miniSkelOn = true;
function toggleMiniSkeleton(){
  miniSkelOn = !miniSkelOn;
  const btn = document.getElementById('mini-skel-btn');
  const overlay = document.getElementById('skeleton-overlay');
  btn.classList.toggle('active', !miniSkelOn);
  btn.title = miniSkelOn ? 'Hide Skeleton' : 'Show Skeleton';
  // show skeleton when mini island is open and skel is enabled
  if(miniIslandOpen && !miniSkelOn){
...
function toggleLauncherFromMini(){
  document.getElementById('app').classList.remove('launcher-hidden');
  document.getElementById('skeleton-overlay').classList.remove('visible');
  document.getElementById('island-cam-center').classList.remove('open');
  miniIslandOpen = false;
  miniCamOn = false;
  document.getElementById('mini-island').classList.remove('open');
  setIslandActive('island-mini-btn', false);
}
...
/* ── Interactable toggles: patch toggleSwitch to mark changes ── */
window.toggleSwitch = function(key){
  const el = document.getElementById('toggle-' + key);
  if(el) el.classList.toggle('on');
  markChanged();
};

/* Sliders: mark changes on user input */
document.addEventListener('input', function(e){
  if(e.target && e.target.type === 'range') markChanged();
});

/* Combos and selects: mark changes */
document.addEventListener('change', function(e){
  if(e.target && e.target.tagName === 'SELECT' && e.target.id !== 'theme-combo') markChanged();
});
